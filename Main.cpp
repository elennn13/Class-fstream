...
int main() {
	...
	if (fs.is_open()) {
		...
		fs.seekg(0, std::ios::beg);//смещение курсора от начала 
	...
	if (datef.is_open()) {
		...
		int first_dot = dates.find('.'); //позиция первой точки (.)
...
bool file_insert(std::string path, int pos, std::string str) {
	...
	if (!fs.is_open())//если открыть не получилось
		return false;

	std::string file, line;// прочтение файла
